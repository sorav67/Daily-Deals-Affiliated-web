...
const getPrisma = () => {
  ...
  if (!globalForPrisma.prisma) {
    ...
    // Clean and Parse the URL manually
      ...
      <nav className="bg-white border-b border-slate-200 sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center">
          <div className="flex items-center gap-2">
            ...
            <span className="font-extrabold text-xl text-slate-900">LootDrop</span>
          </div>
        </div>
      </nav>

      <main className="flex-grow max-w-6xl mx-auto px-4 py-12 w-full">
        <h2 className="text-2xl font-bold text-slate-800 mb-8">Latest Drops</h2>
        {deals.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {deals.map((deal) => (
              <div key={deal.id} className="bg-white rounded-2xl border border-slate-200 overflow-hidden p-6 flex flex-col">
